...
public class StreamTagsOrderTest {
  ...
  public StreamTagsOrderTest() {
    ...
    EventTag eventTag_1 = new EventTag(Util.generateRandomHex64String(), url);
    ...
    EventTag eventTag_2 = new EventTag(Util.generateRandomHex64String(), url);
    ...
    EventTag eventTag_3 = new EventTag(Util.generateRandomHex64String(), url);
  ...
  void equalityTest() {
    ...
    System.out.printf("flatMap tags_1, tags_2:\n  %s%n", getCollect(flatMappedList));
    extracted(concat_1_2, flatMappedList);
  }

  @Test
  void equalityTest_2() {
    List<BaseTag> concat_2_3 = concat(tags_2, tags_3);
    System.out.printf("concat tags_2, tags_3:\n  %s%n", getCollect(concat_2_3));
    List<BaseTag> flatMappedList = flatMap(tags_2, tags_3);
    System.out.printf("flatMap tags_2, tags_3:\n  %s%n", getCollect(flatMappedList));
    extracted(concat_2_3, flatMappedList);
  }

  @Test
  void equalityTest_3() {
    List<BaseTag> concat_1_2_3 = concat(concat(tags_1, tags_2), tags_3);
    System.out.printf("concat_1_2_3:\n  %s%n", getCollect(concat_1_2_3));
    List<BaseTag> flatMappedList = flatMap(flatMap(tags_1, tags_2), tags_3);
    System.out.printf("concat_1_2_3:\n  %s%n", getCollect(flatMappedList));
    extracted(concat_1_2_3, flatMappedList);
  }
  ...
  void equalityTest_4() {
    ...
    System.out.printf("manual_concat_1_2_3:\n  %s%n", getCollect(manual_concat_1_2_3));
    List<BaseTag> flatMappedList = flatMap(flatMap(tags_1, tags_2), tags_3);
    System.out.printf("concat_1_2_3:\n  %s%n", getCollect(flatMappedList));
    extracted(manual_concat_1_2_3, flatMappedList);
  }
  ...
  private @NonNull String getCollect(List<BaseTag> concat_1_2) {
    return getTagStream(concat_1_2).collect(Collectors.joining("\n  "));
  }
  ...
  public static @NonNull Stream<String> getTagStream(List<BaseTag> tags) {
    int maxTagNameLength =
       tags.stream().mapToInt(tag -> tag.getClass().getSimpleName().length()).max().orElse(0);
    int maxHashCodeLength =
       tags.stream().mapToInt(tag -> String.valueOf(tag.hashCode()).length()).max().orElse(0);
